...
extension Parser {
    ...
    mutating func parse(bytes: Span<UInt8>) throws(TOMLError) {
        while token.kind != .eof {
            ...
            case .string:
                try parseKeyValue(bytes: bytes, tableIndex: currentTable)
    ...
    mutating func createKeyTable(bytes: Span<UInt8>, token: Token, inTable tableIndex: Int, implicit: Bool = false) throws(TOMLError) -> Int {
        ...
        switch tableValue(table: tables[tableIndex], key: key) {
    ...
    mutating func parseInlineTable(bytes: Span<UInt8>, tableIndex: Int) throws(TOMLError) {
        ...
        while true {
            ...
            try parseKeyValue(bytes: bytes, tableIndex: tableIndex)
    ...
    mutating func parseKeyValue(bytes: Span<UInt8>, tableIndex: Int) throws(TOMLError) {
        ...
        if token.kind == .dot {
            ...
            if let existingTableIndex = lookupTable(in: table, key: subTableKey) {
                ...
                subTableIndex = try createKeyTable(bytes: bytes, token: key, inTable: tableIndex, implicit: true)
            }

            try nextToken(bytes: bytes, isDotSpecial: true)
            try parseKeyValue(bytes: bytes, tableIndex: subTableIndex)
            return
        }
        ...
        if token.kind == .string {
            let index = try createKeyValue(bytes: bytes, token: key, inTable: tableIndex)
            let value = token
        ...
        if token.kind == .lbracket {
            let index = try createKeyArray(bytes: bytes, token: key, inTable: tableIndex)
            try parseArray(bytes: bytes, arrayIndex: index)
            return
        }

        if token.kind == .lbrace {
            let index = try createKeyTable(bytes: bytes, token: key, inTable: tableIndex)
            try parseInlineTable(bytes: bytes, tableIndex: index)
            return
        }
    ...
    mutating func parseSelect(bytes: Span<UInt8>) throws(TOMLError) {
        ...
        if !llb {
            ...
            currentTable = try createKeyTable(bytes: bytes, token: z, inTable: currentTable)
        } else {
            // [[x.y.z]] -> create z = [] in x.y
            ...
            if maybeArrayIndex == nil {
                maybeArrayIndex = try createKeyArray(bytes: bytes, token: z, inTable: currentTable, kind: .table)
            }
...
extension Token {
    ...
    func unpackFloat(bytes: Span<UInt8>, context: TOMLKey) throws(TOMLError) -> Double {
        ...
        if isdigit(Int32(bytes[index])) == 0 {
            guard (
                ...
                throw TOMLError(.invalidFloat2(context: context, value: self, reason: "Expected nan or inf, found \(bytes[index])"))
            }
            resultCodeUnits.append(bytes[index])
    ...
    func unpackString(bytes: Span<UInt8>, context: TOMLKey) throws(TOMLError) -> String {
        ...
        if bytes.count == 0 {
            throw TOMLError(.invalidString2(context: context, value: self, reason: "missing closing quote"))
        }
        ...
        if quoteChar == CodeUnits.singleQuote {
            do {
                ...
                throw TOMLError(.invalidString2(context: context, value: self, reason: error.localizedDescription))
            }
        } else {
            do {
    ...
    func unpackInteger(bytes: Span<UInt8>, context: TOMLKey) throws(TOMLError) -> Int64 {
        ...
        if bytes[index] == CodeUnits.underscore {
            throw TOMLError(.invalidInteger2(context: context, value: self, reason: "cannot start with a '_'"))
        }
        ...
        if bytes[index] == CodeUnits.number0 {
            ...
            if nextIndex < text.upperBound {
                if bytes[nextIndex] == CodeUnits.lowerX {
                    if hasSign {
                        throw TOMLError(.invalidInteger2(context: context, value: self, reason: "hexadecimal integers cannot have explicit signs"))
                    }
                    base = 16
                    index += 2
                } else if bytes[nextIndex] == CodeUnits.lowerO {
                    if hasSign {
                        throw TOMLError(.invalidInteger2(context: context, value: self, reason: "octal integers cannot have explicit signs"))
                    }
                    base = 8
                    index += 2
                } else if bytes[nextIndex] == CodeUnits.lowerB {
                    if hasSign {
                        throw TOMLError(.invalidInteger2(context: context, value: self, reason: "binary integers cannot have explicit signs"))
                    }
                    base = 2
                    index += 2
                } else if bytes[nextIndex].isDecimalDigit || bytes[nextIndex] == CodeUnits.underscore {
                    throw TOMLError(.invalidInteger2(context: context, value: self, reason: "decimal integers cannot have leading zeros"))
                }
            }
        ...
        while index < text.upperBound {
            ...
            if ch == CodeUnits.underscore {
                guard
                    ...
                    throw TOMLError(.invalidInteger2(context: context, value: self, reason: "cannot use '_' adjacent to a non-digit"))
                }

                if index >= text.endIndex {
                    throw TOMLError(.invalidInteger2(context: context, value: self, reason: "cannot end with a '_'"))
                }

                let next = bytes[index]
                if next == CodeUnits.underscore {
                    throw TOMLError(.invalidInteger2(context: context, value: self, reason: "cannot contain consecutive '_'"))
                }
            ...
            guard isValidDigit(ch, base: base) else {
                throw TOMLError(.invalidInteger2(context: context, value: self, reason: "invalid digit for base \(base)"))
            }
        ...
        guard let i = Int64(s, radix: base) else {
            throw TOMLError(.invalidInteger2(context: context, value: self, reason: "\(s) is a invalid integer of base \(base)"))
        }
        return i
    }
    ...
    func unpackAnyValue(bytes: Span<UInt8>, context: TOMLKey) throws(TOMLError) -> Any {
        ...
        guard firstChar?.isDecimalDigit == true else {
            throw TOMLError(.invalidValueInTable2(context: context, token: self))
        }
...
extension Parser {
    ...
    @available(iOS 12, macOS 13, watchOS 4, tvOS 12, visionOS 1, *)
    mutating func createKeyValue(bytes: UnsafeBufferPointer<UInt8>, token: Token, inTable tableIndex: Int) throws(TOMLError) -> Int {
        let key = try normalizeKey(bytes: bytes, token: token, keyTransform: keyTransform)
        if tableValue(table: tables[tableIndex], key: key) != nil {
            throw TOMLError(.badKey(lineNumber: token.lineNumber))
        }
    ...
    @available(iOS 12, macOS 13, watchOS 4, tvOS 12, visionOS 1, *)
    mutating func createKeyTable(bytes: UnsafeBufferPointer<UInt8>, token: Token, inTable tableIndex: Int, implicit: Bool = false) throws(TOMLError) -> Int {
        let key = try normalizeKey(bytes: bytes, token: token, keyTransform: keyTransform)
    ...
    @available(iOS 12, macOS 13, watchOS 4, tvOS 12, visionOS 1, *)
    mutating func createKeyArray(bytes: UnsafeBufferPointer<UInt8>, token: Token, inTable tableIndex: Int, kind: InternalTOMLArray.Kind? = nil) throws(TOMLError) -> Int {
        let key = try normalizeKey(bytes: bytes, token: token, keyTransform: keyTransform)
        if tableValue(table: tables[tableIndex], key: key) != nil {
            throw TOMLError(.keyExists(lineNumber: token.lineNumber))
        }
    ...
    @available(iOS 12, macOS 13, watchOS 4, tvOS 12, visionOS 1, *)
    mutating func parseKeyValue(bytes: UnsafeBufferPointer<UInt8>, tableIndex: Int) throws(TOMLError) {
        let table = tables[tableIndex]
        if table.readOnly {
            throw TOMLError(.syntax(lineNumber: token.lineNumber, message: "cannot insert new entry into existing table"))
        }
...
extension Parser {
    func tableValue(table: InternalTOMLTable, key: String) -> InternalTOMLTable.Value? {
        ...
        for arr in table.arrays {
            if arrays[arr].key == key {
                return .array(arr)
            }
        }

        for table in table.tables {
            if tables[table].key == key {
                return .table(table)
            }
        }
