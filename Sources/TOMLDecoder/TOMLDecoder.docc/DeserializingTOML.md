...
consisting of different leaf data types like an integer,
or a local date;
and structural types like a table or an array.
If these sound unfamiliar to you,
...
and some logic that converts the TOML text into these data types
...
TOMLDecoder picks a set of Swift types to represent each TOML data type.
...
In that case, check out <doc:DecodingTOML>_.
...
TOMLDecoder APIs in this layer reflect this strongly typed relation.
...
"arrays of tables" (a ``TOMLArray`` can contain ``TOMLTable``s).
...
Therefore, the APIs allow retrieving a ``LocalDate`` or a ``LocalTime``
...
The APIs for the parser reflect strong type relations.
...
And the return value is an `Int64`.

As the only other structural type,
``TOMLArray`` has a similar design when it comes to types.
...
what structures contain what, etc.
...
needed for parsing.
In practice,
the boundary between the 2 isn't as clear cut as stated.
But, it helps.

Since both phases contain failure points,
the APIs for them require you to `try`.
