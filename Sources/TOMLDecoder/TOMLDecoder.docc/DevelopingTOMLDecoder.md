...
* A subset of tests is generated
...
* Generated source files should have a `.Generated.swift` suffix in the filename.
...
In practice, this usually means the comment atop `Package.swift` needs to
...
For TOMLDecoder,
this usually is controlled by the `platforms` field in `Package.swift`.
